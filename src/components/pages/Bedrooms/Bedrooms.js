...
const ImageGallery = () => {
    ...
    const imageCards = data.Bedrooms.map((image) => (
    ...
    const showNext = (e) => {
        e.stopPropagation();
        let currentIndex = data.Bedrooms.indexOf(imageToShow);
        if (currentIndex >= data.Bedrooms.length - 1) {
            setLightBoxDisplay(false);
        } else {
            let nextImage = data.Bedrooms[currentIndex + 1];
